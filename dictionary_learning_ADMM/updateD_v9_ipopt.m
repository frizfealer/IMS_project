function [ DI, kappa ] = updateD_v9_ipopt( InputData, outW, outW0, DI, verbose, varargin )
%--------------------------------------------------------------------------
%updateD_v9_ipopt: update the dictionary with dictionary template
%--------------------------------------------------------------------------
% DESCRIPTION: using the interior point method
%
% INPUT ARGUMENTS:
%   InputData, the InputData data structure.
%   outW, outW0, the abundace matrix.
%   DI, the dictionary interior data sturcutre, can be initialized by
%   initDInterior
%   verbose, whether to have verbose output
%   varargin, additional parameters, under construction
% OUTPUT ARGUMENTS: 
%   DI, the updated dictionary interior data sturcutre
%   kappa, the parameter in the negative binomial model.

%% pour out data
inY = InputData.dataCube;
scaleFac = InputData.scaleFactor;
validMap = InputData.BlkDS.indMap .* InputData.aMatrix;
aIdx = validMap==1;
Y = sparse(inY(:, aIdx));
W = outW(:, aIdx);
[sLen, mLen] = size(DI.D);
...
% kappa for negative-binomial, under construction
kappa = [];
ins = max( W, [], 2 );
usedMolIdx = find( ins > max(W(:))* DI.W_LOWER_BOUND );
rMLen = length( usedMolIdx );
W = W( usedMolIdx, : );
rD = full( DI.D( :, usedMolIdx ) );
DTemplate = DI.DTemplate(:, usedMolIdx );
...
options.ipopt.max_iter         = DI.itNum;
...
if strcmp( DI.LINK_FUNC, 'log' ) == 1
    funcs.objective         = @objective;
    funcs.gradient          = @gradient;
elseif strcmp( DI.LINK_FUNC, 'identity' ) == 1
    funcs.objective         = @objective_identity;
    funcs.gradient          = @gradient_identity;
elseif strcmp( DI.LINK_FUNC, 'log_gaussain' ) == 1    
    funcs.objective         = @objective_log_gaussain;
    funcs.gradient          = @gradient_log_gaussain;
elseif strcmp( DI.LINK_FUNC, 'negative_binomial' ) == 1
...
if strcmp( DI.CONSTRAINT, 'L2_SQUARE' ) == 1
    funcs.constraints  = @constraints;
    funcs.jacobian          = @jacobian;
    funcs.jacobianstructure = @jacobianstructure;
    %the L1 constraint
elseif strcmp( DI.CONSTRAINT, 'L1' ) == 1
...
options.auxdata = { Y, W, staticTerm, nonZPos, nonZPosY, nonZPosX, DI.phi, scaleFac, nonZGrpInfo, nonZLen, jacobStruct, WT, gRes, nonZIdx, YT, sTermT, gPreY, gEPreY};
if strcmp( DI.LINK_FUNC, 'negative_binomial' ) == 1
    kappa = startKappa;
    preD = zeros( size(rD) );
    it = 1;
    YWT = Y*WT;
...
% preY( preY == 0 ) = 1e-32;
% f = -Y.* preY + ePreY;
lPreY = preY; lPreY(lPreY~=0) = log(lPreY(lPreY~=0));
f = sum( sum( -YT.* lPreY + preY ) ) * scaleFac;
f = f + phi* sum(x);
end
