function [WResStruct] = updateW_ADMM_v4( LINK_FUNC, Y, D, aMatrix, itNum, lambda, theta, L1Flag, logFY, initVar, scaleFactor, offsetFlag, varargin )
...
for j = 1:BlkDS.blkNum
    ...
    RforUW = genSparseGroupingMatrix2( cRblk, mLen, offsetFlag);
...
%         tic;
        ...
        fprintf( 'updating W... ' );
        if offsetFlag == 0
            [ w_w0 ] = update_w_w0_v4( curZ0, curU0, curZ1, curU1, curZ2, curU2, CforUW, curRho, [curW], offsetFlag );
            tmp = reshape(w_w0, mLen, length(loc));
            curW = tmp(1:(end), :);
        else
            [ w_w0 ] = update_w_w0_v4( curZ0, curU0, curZ1, curU1, curZ2, curU2, CforUW, curRho, [curW; curW0(:)'], offsetFlag );
