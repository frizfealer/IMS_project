function [WResStruct] = updateW_ADMM( Y, D, W, W0, z0, z1, aMatrix, BlkDS, itNum, lambda, phi, theta, scaleFactor, logFY, Rblk, varargin )
...
%% update u0, u1
    ...
    fprintf( '%d: %g %g %g %g %g %g ',itNumADMM, rfn2, epsPri, s0n2, epsDual, curRhoAry(itNumADMM), full(tmp) );
    time = toc;
    fprintf( 'time: %g\n', time );
    LPAryADMM(itNumADMM+1) = LP_DL_Poiss( aMatrix, Y, W, W0, D, lambda, phi, theta, scaleFactor, logFY, 0 );
    % ONE OF the condition must be satisfied to have next big update
