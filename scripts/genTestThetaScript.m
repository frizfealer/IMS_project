function [ ] = genTestThetaScript( folderName, seq )
%genTestLambdaScript generate TestLambdaScript for each seq
mkdir(folderName);
cd(folderName);
for i = 1:length(seq)
    fileName = ['testThetaScript_' num2str(i) '_.m'];
    ...
    fprintf( fileID, ['[df, lambda] = testThetaMax( dataCube, iD, 1, 1, 1e-1, 1,' num2str(seq(i)) ', 5e-2 );\n'] );
